...
use yalland_node_runtime::{GenesisConfig, RuntimeApi};
use sc_service::{
...
macro_rules! new_full_start {
	($config:expr) => {{
		...
		let builder = sc_service::ServiceBuilder::new_full::<
			node_primitives::Block, yalland_node_runtime::RuntimeApi, node_executor::Executor
		>($config)?
...
type ConcreteClient =
	Client<
		...
		ConcreteBlock,
		yalland_node_runtime::RuntimeApi
	>;
#[allow(dead_code)]
...
pub type NodeConfiguration<C> = Configuration<C, GenesisConfig>;
...
mod tests {
	...
	use yalland_node_runtime::constants::{currency::CENTS, time::SLOT_DURATION};
